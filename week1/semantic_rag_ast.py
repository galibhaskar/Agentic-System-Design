...
def chunk_docs(docs: list[Document]) -> list:
    """Chunks the given documents by parsing their AST and extracting class and function definitions.
    """
    # Note: there's no chunk overlap here since we're chunking by logical code blocks, but we could add some if needed.
    ...
    for doc in docs:
        ...
        try:
            tree = ast.parse(code)
            # Process the AST nodes as needed
        except SyntaxError as e:
            print(f"Error parsing code from {source}: {e}")
            chunks.append(Document(page_content=code,
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ClassDef) and isinstance(parent, ast.Module):
                ...
                if chunk_code:
                    chunks.append(Document(page_content=chunk_code,
...
if __name__ == "__main__":
    ...
    print(f"Created {len(chunks)} chunks using AST parsing.")
