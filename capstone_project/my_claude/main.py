...
from my_claude.memory.short_term import get_session_history, get_checkpointer_db_path
...
async def initialize(checkpointer):
    ...
    return llm, embedder, index, session_id, agent
...
def _render_menu():
    """
    Render the command menu for the user.
    """
...
async def _run_async():
    ...
    async with AsyncSqliteSaver.from_conn_string(get_checkpointer_db_path()) as checkpointer:
        llm, embedder, index, session_id, agent = await initialize(checkpointer)
        ...
        while True:
            ...
            if user_input.lower() == "/exit":
                ...
                response = await handle_query(agent, question, thread_id=session_id)
                ...
                history = await get_session_history(checkpointer, session_id)
                if not history:
